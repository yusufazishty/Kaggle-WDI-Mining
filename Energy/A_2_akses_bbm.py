...
akses_bbm_lines = SQLContext.read.format('json').load("fetched_data/akses_bbm.json")
akses_bbm_rdd = akses_bbm_lines.map(transform_data)
akses_bbm_rdd.cache()
akses_bbm_rdd.collect()
print("RDD Data collected")
akses_bbm_rdd.foreach(println)

#Map the values
akses_bbm_maped = akses_bbm_rdd.mapValues(lambda x: (x,1))
akses_bbm_maped.cache()
akses_bbm_maped.collect()
print("Mapped RDD Data")
akses_bbm_maped.foreach(println)

#Reduced By Key
akses_bbm_reduced = akses_bbm_maped.reduceByKey(lambda x, y: (x[0] + y[0], x[1] + y[1]))
akses_bbm_reduced.cache()
akses_bbm_reduced.collect()
print("Reduced RDD Data")
akses_bbm_reduced.foreach(println)

#Get The Average
akses_bbm_final_maped = akses_bbm_reduced.mapValues(lambda x: x[0] / x[1])
akses_bbm_final_maped.cache()
akses_bbm_final_maped.collect()
print("Average per Country collected")
akses_bbm_final_maped.foreach(println)

# Make Vector the data
autoVector = akses_bbm_final_maped.map(transformToVector)
autoVector.persist()
autoVector.collect()
...
list_akses_bbm_final_maped = akses_bbm_final_maped.collect()
list_akses_bbm_rdd = akses_bbm_rdd.collect()
...
bbm_lines=akses_bbm_lines.collect()
CountryCode=[]
Country=[]
for i in range(len(bbm_lines)):
    if bbm_lines[i].CountryCode not in CountryCode:
        CountryCode.append(bbm_lines[i].CountryCode)
    if bbm_lines[i].Country not in Country:
        Country.append(bbm_lines[i].Country)    
    
data_result_predicted=[]
for i in range(akses_bbm_final_maped.count()):
    NameIdx = CountryCode.index(list_akses_bbm_final_maped[i][0])
    CountryName = Country[NameIdx]
    data=[str(list_akses_bbm_final_maped[i][0]), CountryName, float(list_predictions[i][0]), int(list_predictions[i][1])]
...
save_txt(data_result_predicted, "A_2_bbm_cluster.txt")
print("A_2_bbm_cluster.txt saved")
