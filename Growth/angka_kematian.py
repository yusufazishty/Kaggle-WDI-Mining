...
conf.setAppName("angka_kematian_clustering")
...
angka_kematian_lines = SQLContext.read.format('json').load("fetched_data/angka_kematian.json")
angka_kematian_rdd = angka_kematian_lines.map(transform_data)
angka_kematian_rdd.cache()
angka_kematian_rdd.collect()
print("RDD Data collected")
angka_kematian_rdd.foreach(println)

#Map the values
angka_kematian_maped = angka_kematian_rdd.mapValues(lambda x: (x,1))
angka_kematian_maped.cache()
angka_kematian_maped.collect()
print("Mapped RDD Data")
angka_kematian_maped.foreach(println)

#Reduced By Key
angka_kematian_reduced = angka_kematian_maped.reduceByKey(lambda x, y: (x[0] + y[0], x[1] + y[1]))
angka_kematian_reduced.cache()
angka_kematian_reduced.collect()
print("Reduced RDD Data")
angka_kematian_reduced.foreach(println)

#Get The Average
angka_kematian_final_maped = angka_kematian_reduced.mapValues(lambda x: x[0] / x[1])
angka_kematian_final_maped.cache()
angka_kematian_final_maped.collect()
print("Average per Country collected")
#angka_kematian_final_maped.foreach(println)

# Make Vector the data
autoVector = angka_kematian_final_maped.map(transformToVector)
autoVector.persist()
autoVector.collect()
...
list_angka_kematian_final_maped = angka_kematian_final_maped.collect()
list_angka_kematian_rdd = angka_kematian_rdd.collect()
...
listrik_lines=angka_kematian_lines.collect()
CountryCode=[]
Country=[]
...
for i in range(angka_kematian_final_maped.count()):
    NameIdx = CountryCode.index(list_angka_kematian_final_maped[i][0])
    CountryName = Country[NameIdx]
    data=[str(list_angka_kematian_final_maped[i][0]), CountryName, float(list_predictions[i][0]), int(list_predictions[i][1])]
...
save_txt(data_result_predicted, "angka_kematian_cluster.txt")
print("angka_kematian_cluster.txt saved")
